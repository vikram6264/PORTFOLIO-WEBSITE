...
export default function ProjectCard({ project, index }) {
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-xl aspect-[16/10] bg-bg-secondary mb-4">
        <div className="absolute inset-0 bg-gradient-to-br from-primary-500/20 via-bg-card to-secondary-500/20" />
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-4xl sm:text-5xl font-bold text-text-muted/20 group-hover:text-primary-500/30 transition-colors duration-500">
            {project.title.split(' ').map(w => w[0]).join('').slice(0, 3)}
          </span>
        </div>

        {project.featured && (
          <div className="absolute top-3 left-3 badge-primary text-xs">
            Featured
          </div>
        )}
      ...
      <div className="flex-1 flex flex-col p-1">
        ...
        <div className="mt-auto flex items-center gap-3 pt-2">
          <a
            ...
            className="flex items-center gap-1.5 text-xs text-text-muted hover:text-primary-400 transition-colors"
          >
          ...
          <button
            ...
            className="ml-auto flex items-center gap-1 text-xs text-primary-400 hover:text-primary-300 font-medium transition-colors"
          >
